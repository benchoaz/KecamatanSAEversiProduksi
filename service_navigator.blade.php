            ...
            <div id="snNodePanel">
                ...
                <div class="sn-node-grid" id="snNodeGrid">
                    ...
                    <button class="sn-node-card"
                            onclick="selectNode({{ $node->id }}, '{{ rawurlencode($node->name) }}', {{ $node->is_leaf ? 'true' : 'false' }}, {{ $node->show_identity_form ? 'true' : 'false' }}, '{{ rawurlencode($node->requirement_text ?? '') }}')">
...
async function loadLeafForm(nodeId, nodeName, showIdentity = true, sopText = '') {
    ...
    if (idSectionEl) {
        const idLabel = idSectionEl.querySelector('.sn-form-section-label');
        const nameInput = document.querySelector('input[name="nama_pemohon"]');
        const nikInput = document.querySelector('input[name="nik"]');
        
        if (idLabel && nameInput && nikInput) {
            const nameLabel = nameInput.previousElementSibling;
            const nikLabel = nikInput.previousElementSibling;
            
            if (isAnak) {
                idLabel.innerHTML = '<i class="fas fa-child"></i> Data Anak';
                if (nameLabel) nameLabel.innerHTML = 'Nama Anak <span>*</span>';
                nameInput.placeholder = 'Nama Lengkap Anak';
                if (nikLabel) nikLabel.innerHTML = 'NIK Anak / NIK Kepala Keluarga <span>*</span>';
            } else {
                idLabel.innerHTML = '<i class="fas fa-user-circle"></i> Data Pemohon';
                if (nameLabel) nameLabel.innerHTML = 'Nama Lengkap <span>*</span>';
                nameInput.placeholder = 'Sesuai KTP';
