import os
import pty
import time

host = '43.134.166.153'
user = 'ubuntu'
pw = 'nebula-57@-ocean'

def run_ssh_cmd(cmd):
    pid, fd = pty.fork()
    if pid == 0:
        os.execv('/usr/bin/ssh', ['ssh', '-o', 'StrictHostKeyChecking=no', f'{user}@{host}', cmd])
    else:
        output = b""
        start = time.time()
        while time.time() - start < 120:
            try:
                chunk = os.read(fd, 4096)
                if not chunk: break
                output += chunk
                if b"password:" in chunk.lower():
                    os.write(fd, (pw + "\n").encode())
            except:
                break
        return output.decode(errors='ignore')

# 1. Create Directory inside Container
print("--- Creating Directory in Container ---")
print(run_ssh_cmd("sudo -S docker exec -T kecamatan-app mkdir -p /var/www/app/Console/Commands"))

# 2. Copy File into Container
print("--- Copying Command File ---")
src = "/home/ubuntu/kecamatanSAE/app/app/Console/Commands/SyncServiceToFaq.php"
dest = "kecamatan-app:/var/www/app/Console/Commands/"
print(run_ssh_cmd(f"sudo -S docker cp {src} {dest}"))

# 3. Clear Cache
print("--- Clearing Cache ---")
print(run_ssh_cmd("sudo -S docker compose -f /home/ubuntu/kecamatanSAE/docker-compose.vps.yml exec -T app php artisan optimize:clear"))

# 4. Run Sync
print("--- Running FAQ Sync ---")
print(run_ssh_cmd("sudo -S docker compose -f /home/ubuntu/kecamatanSAE/docker-compose.vps.yml exec -T app php artisan faq:sync-services --clear"))

print("--- DONE ---")
