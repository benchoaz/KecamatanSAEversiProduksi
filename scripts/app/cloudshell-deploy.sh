#!/bin/bash
# Cloud Shell deployment - handles both repo root and subdirectory structure

set -e

echo "🚀 KECAMATAN-LAYANAN-WHATSAPP - Cloud Shell Setup"
echo "=================================================="
echo ""

# Check if we're in a subdirectory
if [ ! -f "docker-compose.yml" ] && [ -d "dashboard-kecamatan" ] && [ -f "dashboard-kecamatan/docker-compose.yml" ]; then
    echo "📂 Found dashboard-kecamatan subdirectory, navigating..."
    cd dashboard-kecamatan
elif [ ! -f "docker-compose.yml" ]; then
    echo "❌ Error: docker-compose.yml not found!"
    echo "Please run this script from the project root or dashboard-kecamatan directory"
    exit 1
fi
...
while [ $ATTEMPT -lt $MAX_WAIT ]; do
  ...
  if [ $((ATTEMPT % 10)) -eq 0 ]; then
    echo "    Still waiting... ($ATTEMPT/$MAX_WAIT seconds)"
  fi
  sleep 1
done
...
echo "✨ Happy coding! Your app is now running in Google Cloud Shell."
